...
function createFileUploader(options) {
  if (web.isServer) {
    return {
      files: () => [],
      selectFiles: utils.noop,
      removeFile: utils.noop,
      clearFiles: utils.noop
    };
  }
...
// src/index.ts
exports.fileUploader = (element, options) => {
  if (web.isServer) {
    return;
  }
  ...
  solidJs.onMount(() => {
    ...
    solidJs.onCleanup(() => element.removeEventListener("change", onChange));
