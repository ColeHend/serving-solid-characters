...
// Compute repo root (two levels up from src/SolidCharacters)
var repoRoot = Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), "..", ".."));

builder.Services.AddSpaStaticFiles(config =>
{
    config.RootPath = Path.Combine(repoRoot, "client", "dist");
});
...
var publicPath = Path.Combine(repoRoot, "client", "public");
...
string path = Path.Combine(repoRoot, "client", "dist");
