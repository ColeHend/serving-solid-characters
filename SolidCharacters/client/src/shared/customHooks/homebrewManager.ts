...
class HomebrewManager {
  ...
  // Items
  public addItem = (item: srdItem): Promise<void> | null => { if (this._items().some(i => i.name === item.name)) return null; return new Promise(res => this.addItemToDB(Clone(item)).subscribe({ complete: () => res(), error: () => res() })); }
