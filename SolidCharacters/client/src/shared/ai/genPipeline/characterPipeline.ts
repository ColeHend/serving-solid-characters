 ...
 */
export const CHARACTER_PIPELINE_PHASES = [
...
const PHASES = CHARACTER_PIPELINE_PHASES;
...
/**
 * Run (or RESUME) the Character pipeline. On a fresh run `working` starts empty and all phases run. On
 * resume (plan §9, M6) `working`/`brief` come from the persisted checkpoint and each phase is SKIPPED when
 * its slice of the working object is already present — so a reloaded run finishes from where it stopped
 * without re-rolling scores or re-deciding the foundation. The final compute+assemble phase always runs (it
 * is pure code over the working object). A phase that legitimately produced nothing on the prior run (e.g. a
 * skipped narrative) is simply retried, which is harmless.
 */
export async function runCharacterPipeline(seed: string, host: CharacterPipelineHost, resume?: PipelineResume<WorkingCharacter>): Promise<void> {
    ...
    try {
        // ── Phase 1 — concept anchor (skip if resumed with one) ────────────────
        if (resume?.brief) {
            brief = resume.brief;
        } else {
