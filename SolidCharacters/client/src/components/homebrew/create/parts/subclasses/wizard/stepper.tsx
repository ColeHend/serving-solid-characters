...
import { STEP_META, StepStatus, SUBCLASS_STEPS, SubclassWizardStep } from './wizard.shared';
...
// subclass step enum (distinct enums aren't assignable, so sharing one component would
// force casts at every call site).

interface StepperProps {
  current: SubclassWizardStep;
  status: (step: SubclassWizardStep) => StepStatus;
  onJump: (step: SubclassWizardStep) => void;
}

export const Stepper: Component<StepperProps> = (props) => {
  const stateClass = (step: SubclassWizardStep) => {
  ...
  return (
    <div class={styles.stepper} role="navigation" aria-label="Wizard steps">
      <For each={SUBCLASS_STEPS}>{(step, i) => (
        <>
          <Show when={i() > 0}>
            <div class={`${styles.connector} ${props.status(SUBCLASS_STEPS[i() - 1]) === 'complete' ? styles.connectorDone : ''}`} />
          </Show>
          <button
            type="button"
