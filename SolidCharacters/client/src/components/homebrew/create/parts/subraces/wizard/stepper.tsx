...
import { SUBRACE_STEPS, SubraceWizardStep, STEP_META, StepStatus } from './wizard.shared';
...
// subrace step enum (distinct enums aren't assignable, so sharing one component would
// force casts at every call site).

interface StepperProps {
  current: SubraceWizardStep;
  status: (step: SubraceWizardStep) => StepStatus;
  onJump: (step: SubraceWizardStep) => void;
}

export const Stepper: Component<StepperProps> = (props) => {
  const stateClass = (step: SubraceWizardStep) => {
  ...
  return (
    <div class={styles.stepper} role="navigation" aria-label="Wizard steps">
      <For each={SUBRACE_STEPS}>{(step, i) => (
        <>
          <Show when={i() > 0}>
            <div class={`${styles.connector} ${props.status(SUBRACE_STEPS[i() - 1]) === 'complete' ? styles.connectorDone : ''}`} />
          </Show>
          <button
            type="button"
