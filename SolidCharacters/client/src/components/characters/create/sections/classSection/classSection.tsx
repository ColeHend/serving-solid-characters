...
export const ClassSection: Component = () => {
  ...
  const isSelected = (name: string) => draft.classes.some((c) => c.name === name);
  ...
  const toggleClass = (class5e: Class5E) => {
    if (isSelected(class5e.name)) {
      actions.removeClass(class5e.name);
      return;
    }
    if (draft.classes.length > 0) {
      ...
      const unmet = [class5e, derived.classByName(draft.classes[0].name)]
        ...
        <For each={sortedClasses()}>
          {(class5e) => (
            <button
              type="button"
              class={styles.card}
              classList={{ [styles.cardSelected]: isSelected(class5e.name) }}
