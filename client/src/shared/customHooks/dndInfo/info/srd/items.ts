...
export function useGetSrdItems(version: '2014' | '2024' | 'both' | string): Accessor<Item[]> {
  if ((version === '2014' || version === 'both') && items2014().length === 0 && !loading2014) {
    loading2014 = true;
    const local$ = HttpClient$.toObservable(SrdDB.items.toArray());
    local$.pipe(
      take(1),
      concatMap(cached => cached.length ? of(cached) : fetchItems('2014')),
      tap(list => { if (list?.length) SrdDB.items.bulkPut(list).catch(err => console.error('Error saving 2014 items:', err)); })
    ).subscribe({
  ...
  if ((version === '2024' || version === 'both') && items2024().length === 0 && !loading2024) {
    loading2024 = true;
    const local$ = HttpClient$.toObservable(SrdDB2024.items.toArray());
    local$.pipe(
      take(1),
      concatMap(cached => cached.length ? of(cached) : fetchItems('2024')),
      tap(list => { if (list?.length) SrdDB2024.items.bulkPut(list).catch(err => console.error('Error saving 2024 items:', err)); })
    ).subscribe({
