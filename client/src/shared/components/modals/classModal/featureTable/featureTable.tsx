...
const FeatureTable: Component<Props> = (props) => {
  ...
  const [currentFeature, setCurrentFeature] = createSignal<
  ...
  return (
    <table class={`${styles.table}`}>
      <thead>
        <tr>
          ...
          <For
            each={fixedSorerer(
              Object.keys(
                highestClassSpecificKeyAmountFirstArr()[0].classSpecific
              )
            )}
          >
            {(Specifickey) => (
              <th class={`${styles.small}`}>{toDisplayFormat(Specifickey)}</th>
            )}
          </For>
        </tr>
      </thead>
      <tbody>
        <For
          each={Class().classLevels.sort((a, b) => a.info.level - b.info.level)}
        >
          {(item) => (
            <tr style={{ border: "1px solid white" }}>
              ...
              <For each={fixedSorerer(Object.keys(item.classSpecific))}>
                {(Specifickey) => (
                  <td>
                    <span>
                      {fixedSelectValues(item.classSpecific[Specifickey])}
                    </span>
                  </td>
                )}
              </For>
