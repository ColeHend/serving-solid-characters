...
const Button: Component<Props> = (props)=> {
    ...
    const menuStyle: Accessor<JSX.CSSProperties> = createMemo(()=>({
		...
		useClickOutside(menuRef, ()=>{
			const xClose = Math.abs(sharedHooks.getMouse().x - showMenu().lastX) < 30;
			const yClose = Math.abs(sharedHooks.getMouse().y - showMenu().lastY) < 30;
			if (showMenu().show && !xClose && !yClose) {
    ...
    return (
        <>
            <button
							{...props}
							onClick={(e)=>{
								if (!!props.menuItems?.length) setShowMenu({show: !showMenu().show, lastX: e.clientX, lastY: e.clientY});
