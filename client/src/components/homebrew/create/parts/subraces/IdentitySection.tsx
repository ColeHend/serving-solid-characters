  ...
  return <FlatCard icon='identity_platform' headerName='Idenity' startOpen={true}>
      <div class="inlineRow inlineDense">
        <FormField name="Subrace Name">
          <Input transparent value={draft()!.name} onInput={e=> updateDraft('name', e.currentTarget.value)} />
        </FormField>
        <FormField name="Speed">
          ...
          style={{ width:'90px' }} 
          value={draft()!.speed} 
          onInput={e=> updateDraft('speed', parseInt(e.currentTarget.value||'0'))} />
        </FormField>
      </div>
      <FormField name="Short Description">
        <TextArea transparent rows={2} text={() => draft()!.text.desc} setText={v => updateDraft('text', { ...draft()!.text, desc: typeof v === 'function'? v(draft()!.text.desc): v }) as any} /></FormField>
      <div class="inlineRow inlineDense">
